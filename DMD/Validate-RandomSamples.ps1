...
[xml]$vtmXml = Get-Content "$XmlPath\f_vtm2_3061125.xml"
...
[xml]$vmpXml = Get-Content "$XmlPath\f_vmp2_3061125.xml"
...
foreach ($vmp in $vmpSamples) {
    ...
    if ($dbResult) {
        $dbVtmid = if ($dbResult.vtmid) { $dbResult.vtmid } else { "NULL" }
...
[xml]$ampXml = Get-Content "$XmlPath\f_amp2_3061125.xml"
...
[xml]$ingXml = Get-Content "$XmlPath\f_ingredient2_3061125.xml"
