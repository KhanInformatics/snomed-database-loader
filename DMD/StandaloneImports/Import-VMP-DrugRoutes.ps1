...
# ===== VMP Drug Routes Import =====
Write-Host "`n===== VMP Drug Routes Import =====" -ForegroundColor Cyan
...
Write-Host "Checking for existing VMP Drug Route records..."
$existingKeys = Get-ExistingKeys -TableName "vmp_drugroute" -KeyColumns @("vpid", "routecd")
Write-Host "  Found $($existingKeys.Count) existing VMP Drug Route records"

Write-Host "Processing VMP Drug Route records..."
$values = @()
$duplicatesSkipped = 0

foreach ($route in $vmpXml.VIRTUAL_MED_PRODUCTS.DRUG_ROUTE.DROUTE) {
    $vpid = [string]$route.VPID
    $routecd = [string]$route.ROUTECD
    
    if ([string]::IsNullOrWhiteSpace($vpid) -or [string]::IsNullOrWhiteSpace($routecd)) { continue }
    ...
    $values += "($(Escape-SqlValue $vpid),$(Escape-SqlValue $routecd))"
}

Write-Host "  Processed $($values.Count) unique VMP Drug Route records"
if ($duplicatesSkipped -gt 0) {
    Write-Host "  Skipped $duplicatesSkipped duplicate VMP Drug Route records"
}

if ($values.Count -gt 0) {
    $columns = "(vpid,routecd)"
    Invoke-BatchedInsert -TableName "vmp_drugroute" -Columns $columns -Values $values -BatchSize $BatchSize
    Write-Host "`nVMP Drug Route import complete!" -ForegroundColor Green
    
    $finalCount = Invoke-Sqlcmd -ServerInstance $ServerInstance -Database $DatabaseName -TrustServerCertificate -Query "SELECT COUNT(*) as cnt FROM vmp_drugroute" -QueryTimeout 30
    Write-Host "Total VMP Drug Route records in database: $($finalCount.cnt)" -ForegroundColor Green
} else {
    Write-Warning "No new VMP Drug Route records to import."
}
